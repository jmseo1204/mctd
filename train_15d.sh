#!/usr/bin/env bash
# train_15d.sh
# Trains the 15D qpos-only AntMaze model (network_size=128, 8 layers)
...
echo "========================================" | tee -a "$LOG_FILE"
echo "[$(date)] Starting 15D qpos-only AntMaze model training" | tee -a "$LOG_FILE"
...
while [ $attempt -lt $MAX_RETRIES ]; do
    ...
    done < <(find "$PROJECT_DIR/outputs" -name "last.ckpt" ! -path "*/downloaded/*" -path "*Train_15D*" 2>/dev/null)

    BASE_CMD="python main.py \
        experiment.tasks=[training] \
        experiment=exp_planning \
        algorithm=df_planning_15d \
        dataset=og_antmaze_giant_navigate_15d \
        +name=Train_15D_antmaze \
        wandb.mode=offline \
